...
const PopupReserveSwapLater = ({ deposit, remainingAmount, duration, token, confirmPayLater, index }) => {
    ...
    return (
        <div className={`${classes.root}`}>
            ...
            <div className='radius-5 flex-justify' style={{ background: "rgba(255, 0, 131, 0.05)", padding: "10px" }}>
                <Checkbox className={`primary-text`} {...label} sx={{ verticalAlign: "top", padding: "0 10px", marginTop: "-21px" }} />
                <span className='font-10 t2-text opacity'>I understand that this is a reserve now, pay later swap option. By paying the reservation price, I understand that in order to complete the swap, I must pay the balance within the time period. If I do not pay, I lose the reservation price as a penalty.</span>
            </div>

            <div>
                <div className='margin-tb-10 float-right'>
                    <div className="">
                        <Button disableRipple className={"btn font-bold-16"} variant="contained" endIcon={<ArrowRightAlt />} onClick={e => confirmPayLater(index)}>Reserve Now</Button>
                    </div>
                </div>
            </div>
        </div>
...
export default PopupReserveSwapLater;